    ...
    def validate_identificationcard(self, data):
        ...
        if usertype == "teacher" and identificationcard is None:
            raise serializers.ValidationError(
                "A teacher must provide an identification card."
            )

        if (
            usertype == "teacher"
            and identificationcard is not None
            and len(identificationcard) != 14
        ):
            raise serializers.ValidationError(
                "Identification card must be exactly 14 digits long."
            )
